...
public class SmtpAgent {
    ...
    public boolean sendMessage() {
        ...
        try {
            ...
            // 첨부파일을 비동기적으로 처리하기 위한 CompletableFuture 리스트 생성
            List<CompletableFuture<Void>> attachmentFutures = new ArrayList<>();

            // 각 첨부파일에 대해 CompletableFuture 생성 및 시작
            for (String filePath : attachments) {
                ...
                // CompletableFuture 리스트에 추가
                attachmentFutures.add(attachmentFuture);
            }

            // 모든 첨부파일 처리가 완료될 때까지 대기
            CompletableFuture<Void> allOf = CompletableFuture.allOf(attachmentFutures.toArray(new CompletableFuture[0]));
            allOf.join();
